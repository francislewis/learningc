...
int main(int argc, char** argv) {
  ...
  mpca_lang(MPCA_LANG_DEFAULT,
    "                                              \
      ...
      toasty  : /^/ <expr>* /$/ ;                  \
    ",
    Number, Symbol, String, Comment, Sexpr, Qexpr, Expr, Toasty);
  ...
  if (argc == 1) {
    ...
    puts("Toasty Version 0.0.0.1.0");
    puts("Press Ctrl+c to Exit\n");
  
    while (1) {
    
      char* input = readline("toasty> ");
      add_history(input);
      
      mpc_result_t r;
      if (mpc_parse("<stdin>", input, Toasty, &r)) {
